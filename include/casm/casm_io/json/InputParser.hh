...
/// this can be any
///   object in the JSON document from which all subsequent paths with be
///   relative to).
/// - self: which is a reference to the JSON subobject
/// `this->input.find_at(this->path)` that will
///   be parsed when member functions (require, optional, etc.) are called and
///   which any error or warning messages they create are referenced to.
///
/// Basic example:
/// \code
...
/// will be false assert(!other_parser.valid()}; \endcode
///
/// Similar options allow returning parsed values instead of assigning,
/// optionally parsing if values exists in the JSON, specifying default values
/// if no value exists, etc.
///
...
struct KwargsParser : public Validator {
  ...
  /// Reference to the JSON component to be parsed, if it exists, otherwise set
  /// to this->input
  ///
  /// Note:
  ...
  /// Default empty, can be used to differentiate between parsers when multiple
  /// values are parsed from a single JSON object
  ...
  /// \param _required If true, insert an error if JSON component refered to by
  /// _path does not exist
  ///
  /// If required to exist, but does not, the KwargsParser will be constructed
  /// and the following error will be inserted:
  ...
  /// Return a fs::path from the top-level to the parent JSON object of
  /// this->self
  ///
  /// The result satisfies:
  /// `this->parent()==this->input.find_at(this->parent_path())`
  ...
  ///
  /// When KwargsParser is contructed with a path that does not exist, then
  /// this->self=this->input.
  bool exists() const;
  ...
  /// Insert a subparser
  ///
  /// Subparsers are stored in a map of path (from this->input) to the
  /// subparser. After being inserted, the subparser's errors and warnings are
  /// included in validation checks
  /// (`valid`) and in parser output (`make_report`, `print_warnings`,
  /// `print_errors`, etc.).
 ...
 private:
  ...
  /// Can use static_cast to get values from InputParser subparsers if they are
  /// included.
...
///
/// The `parse` function should call methods of InputParser (i.e. require,
/// optional, subparse, etc.) to automate the handling of errors and capture of
/// error and warning messages.
///
/// The `parse` function is called when InputParser<T> is constructed from input
/// json:
...
/// warnings, with the path to the location in the json where the issue
/// occurred, do:
///     Log& log = ...;
///     if(!parser.valid()) {
...
/// `parser.value` as an empty unique_ptr<T>, otherwise parser.value should hold
...
class InputParser : public KwargsParser {
 public:
  /// Store the object being read from JSON, use unique_ptr so default
  ...
  /// - "Error: could not construct type '<type_name<RequiredType>()>' from
  /// option '<option>'. <
  ...
  /// Require self.find_at(option) of type RequiredType, assigning result to
  /// value
  ///
  /// Failing to parse the component will result in errors of type:
  /// - "Error: missing required option '<option>'"
  ...
  /// Check that if self.find_at(option) exists it can constructed as type
  /// RequiredType, returning result in unique_ptr
  ///
  /// Failing to parse the component will result in errors of type:
  /// - "Error: could not construct type '<type_name<RequiredType>()>' from
  /// option '<option>'. <
  ...
  /// Check that if self.find_at(option) exists it can constructed as type
  /// RequiredType, assigning result to value
  ///
  /// Failing to parse the component will result in errors of type:
  /// - "Error: could not construct type '<type_name<RequiredType>()>' from
  /// option '<option>'. <
  ///    singleline_help<RequiredType>()>"
  ///
  /// If self.find_at(option) does not exist, do not change `value` and do not
  /// insert an error.
  template <typename RequiredType, typename... Args>
  void optional(RequiredType &value, fs::path option, Args &&...args);

  /// Check for self.find_at(option), return value or default, error if cannot
  /// be constructed
  ///
  /// Failing to parse the component will result in errors of type:
  /// - "Error: could not construct type '<type_name<RequiredType>()>' from
  ...
  ///    singleline_help<RequiredType>()>"
  ///
  /// If self.find_at(option) does not exist, return `_default` and do not
  /// insert an error.
  template <typename RequiredType, typename... Args>
  RequiredType optional_else(fs::path option, const RequiredType &_default,
                             Args &&...args);

  /// Check for self.find_at(option), assign result or default, error if cannot
  /// be constructed
  ///
  /// Failing to parse the component will result in errors of type:
  /// - "Error: could not construct type '<type_name<RequiredType>()>' from
  ...
  ///    singleline_help<RequiredType>()>"
  ///
  /// If self.find(option) does not exist, assign `_default` to `value` and do
  /// not insert an error.
  template <typename RequiredType, typename... Args>
  void optional_else(RequiredType &value, fs::path option,
  ...
  /// \endcode
  template <typename RequiredType, typename... Args>
  std::shared_ptr<InputParser<RequiredType>> subparse(fs::path option,
                                                      Args &&...args);
  ...
  template <typename RequiredType, typename... Args>
  std::shared_ptr<InputParser<RequiredType>> subparse_if(fs::path option,
                                                         Args &&...args);

  /// Subparse, if `this->path / option` exists, else the result->value will be
  /// copy-constructed from `_default`
  template <typename RequiredType, typename... Args>
  std::shared_ptr<InputParser<RequiredType>> subparse_else(
...
/// but instead is parsed with multiple subparsers
